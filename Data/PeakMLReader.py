...
import Data.PeakMLObject as PML
import base64
import numpy as np
...
def add_annotations(parent_element, parent_object):
    for annotation_element in parent_element.findall("./annotations/annotation"):
        ...
        annotation = PML.Annotation(unit_attribute, ontologyref_attribute, label.text, value.text, valuetype.text)
...
def add_peaks(parent_element, peakset):
    for peak_element in parent_element.findall("./peaks/peak"):
        ...
        if peakdata_element is not None:
            ...
            peak_data = PML.PeakData(peakdata_type_attribute, peakdata_size_attribute)
        ...
        peak = PML.Peak(type_attribute, scan, retentiontime, mass, intensity, measurementid, patternid, sha1sum, signal, peak_data)
...
def importElementTreeFromPeakMLFile(tree_data):
    ...
    header_obj = PML.Header(nrpeaks.text, date.text, owner.text, description.text)
    ...
    for set_element in header_element.findall("./sets/set"):
        ...
        set = PML.SetInfo(id.text, type.text, measurementids.text)
    ...
    for sample_element in header_element.findall("./samples/sample"):
        ...
        sample = PML.SampleInfo(id.text, annotations.text)
    ...
    for application_element in header_element.findall("./applications/application"):
        ...
        application = PML.ApplicationInfo(name.text, version.text, date.text, parameters.text)
    ...
    for measurement_element in header_element.findall("./measurements/measurement"):
        ...
        measurement = PML.MeasurementInfo(id.text, label.text, sampleid.text)
        ...
        for scan_element in measurement_element.findall("./scans/scan"):
            ...
            scan = PML.ScanInfo(polarity.text, retentiontime.text)
        ...
        for file_element in measurement_element.findall("./files/file"):
            ...
            file = PML.FileInfo(label.text, name.text, location.text)
    ...
    return PML.PeakML(header_obj, peakset)
